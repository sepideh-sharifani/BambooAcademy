...
const LOGINEMPLOYEE_URL = '/api/auth/employee/login'
...
const Login = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await instance.post(LOGINEMPLOYEE_URL, user,);
            console.log(response)
            //clear input fields
            setEmail('');
            setPassword('');
            localStorage.setItem('admintoken', JSON.stringify(response.data.result.jwtToken))
            localStorage.setItem('admin_Data', JSON.stringify(response.data.result.studentModel))
            setSuccess(true);
        } catch (error: any) {
                        ...
                        <div
                            ...
                            <div className='relative flex flex-col justify-between w-full h-full mx-5 animate-slideUp md:w-2/3'>
                                ...
                                <form
                                    ...
                                    <Link href={"/register/registerEmployee"}><span

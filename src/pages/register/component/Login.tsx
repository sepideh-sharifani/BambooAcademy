        ...
        <div className='flex flex-col items-center justify-center w-1/2 h-auto m-auto overflow-hidden bg-white rounded shadow-lg md:w-full md:flex-row'>
          <div className='relative w-full h-full md:block'>
            ...
            <div className='absolute -translate-x-1/2 divide-y divide-white top-1/2 left-1/2 -tranlate-y-1/2 md:top-1/3 md:left-1/2 md:-translate-x-2/4 md:-tranlate-y-2/4'>
              <div className='mb-4 text-xl text-center text-white divide-y sm:text-2xl'>آکادمی آموزشی بامبو</div>
          ...
          <div className='relative flex flex-col justify-between w-full h-full mx-5 animate-slideUp md:w-2/3'>
            ...
            <form className='flex flex-col items-center justify-between mx-5 py-4' onSubmit={handleSubmit(onSubmit)}>
              {/* email validatation */}
              <input type="email" {...register("email")} className='w-full px-1 py-2 mb-2 bg-gray-100 rounded-sm outline-none focus:border-b-2 focus:border-greenishBlue placeholder:text-xs' placeholder='ایمیل:' />
              <p className="text-xs text-red-700">{errors.email?.message?.toString()}</p>
              {/* password, icon , validate */}
              <div className="relative w-full">
                <input dir='ltr' type={showPassword ? "text" : "password"} {...register("Password")} className='w-full px-1 py-2 my-2 bg-gray-100 rounded-sm outline-none focus:border-b-2 focus:border-greenishBlue placeholder:text-xs' placeholder='رمز عبور*:' />
              ...
              <div className='flex flex-row flex-wrap items-center justify-between w-full my-4'>
                <span className='text-sm'>مرا به خاطر بسپار<input className='mx-2' type="checkbox" /></span>
                <Link to={"/register/reset"}><p className='text-sm text-gray-400 hover:text-black'>فراموشی رمز</p></Link>
              </div>
              <button type='submit' onSubmit={handleSubmit(onSubmit)} className='w-1/2 px-4 py-2 my-4 text-center text-white shadow-2xl border border-transparent rounded cursor-pointer bg-greenishBlue hover:bg-gray-200 hover:text-black hover:border-greenishBlue'>ورود</button>
              <Link to={"/register/register"}><span className='text-xs'>هنوز ثبت نام نکرده اید؟</span></Link>
            </form>
          </div>
        </div>
      </div>
